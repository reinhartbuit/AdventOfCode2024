...
for(let i = 0; i < rows.length ; i+=4){
    ...
    let prize_X = Number(rows[i+2].substring(rows[i+2].indexOf('X=')+2,rows[i+2].indexOf(','))) * 10000000000000;
    let prize_Y = Number(rows[i+2].substring(rows[i+2].indexOf('Y=')+2)) * 10000000000000;

    let totalPrize = 0
    let maxBtnB_X = Math.floor(prize_X/btnB_X)
    let maxBtnB_Y = Math.floor(prize_Y/btnB_Y)

    let maxBtnB =   (maxBtnB_X < maxBtnB_Y ?  maxBtnB_X : maxBtnB_Y)
